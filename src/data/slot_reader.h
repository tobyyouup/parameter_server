...
class SlotReader {
 public:
  ...
  //add by tanxu
  std::unordered_map<uint64, uint64> index_optimized_by_tanxu(int slot_id);
  ...
  void clear(int slot_id) {
    ...
    index_cache_opimized_by_tanxu_.erase(slot_id);
  }
 ...
 private:
  ...
  //add by tanxu
  std::unordered_map<int, std::unordered_map<uint64, uint64>> index_cache_opimized_by_tanxu_;
};
