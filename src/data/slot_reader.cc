...
bool SlotReader::readOneFile(const DataConfig& data, int ith_file) {
  ...
  // Note by Jun:
  //  Potential stack overflow, so replace it 
  //with a vector to make sure big memory consumption
  //will be over heap rather than stack.
  vector<VSlot> vslots;
  vslots.resize(kSlotIDmax);
  uint32 num_ex = 0;
  Example ex;
...
//added by tanxu
std::unordered_map<uint64, uint64> SlotReader::index_optimized_by_tanxu(int slot_id) {
  auto nnz = nnzEle(slot_id);
  if (nnz == 0) return std::unordered_map<uint64, uint64>();

  //std::unordered_map<uint64, uint64> idx = index_cache_opimized_by_tanxu_[slot_id];
  //if (idx.size() == nnz) return idx;

  std::unordered_map<uint64, uint64> idx;
  uint64 key_size = 0;
  for (int i = 0; i < data_.file_size(); ++i) {
    string file = cacheName(ithFile(data_, i), slot_id) + ".colidx";
    SArray<char> comp;
    ...
    for (size_t j = 0; j < uncomp.size(); ++j){
    	key_size += 1;
    	if(idx.find(uncomp[j]) == idx.end()){
    		idx[uncomp[j]] = 1;
		}
		else{
			idx[uncomp[j]] += 1;
