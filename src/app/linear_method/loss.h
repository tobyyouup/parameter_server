...
class ScalarLoss : public Loss<T> {
 public:
  ...
  virtual void compute(const EArray& y, const EArray& y_w, const MatrixPtr<T>& X, const EArray& Xw,
  ...
  void compute(const MatrixPtrList<T>& data, MatrixPtrList<T> gradients) {
    ...
    auto y = data[0]->value();
    auto y_w = (data.size() > 3) ? data[1]->value() : SArray<T>();
    auto X = (data.size() > 3) ? data[2] : data[1];
    auto Xw = (data.size() > 3) ? data[3]->value() : data[2]->value();
    ...
    if (!y.size()) return;
    compute(y.EigenArray(), 
      y_w.EigenArray(), 
      X, 
      Xw.EigenArray(), 
      gradient.EigenArray(), 
      diag_hessian.EigenArray());
  }
...
class LogitLoss : public BinaryClassificationLoss<T> {
 ...
 public:
  ...
  void compute(const EArrayMap& y, const EArrayMap& y_w, const MatrixPtr<T>& X, const EArrayMap& Xw,
    ...
    if (gradient.size()) {
      if (y_w.size()) {
        gradient = X->transTimes( -y * tau * y_w);
      } else {
...
class SquareHingeLoss : public BinaryClassificationLoss<T> {
 public:
  ...
  void compute(const EArrayMap& y, const EArrayMap& y_w, const MatrixPtr<T>& X, const EArrayMap& Xw,
               EArrayMap gradient, EArrayMap diag_hessian) {
    // Note by Jun:
    //  Currently sample weight is only supported for LogitLoss.
    CHECK_EQ(y_w.size(), 0);
