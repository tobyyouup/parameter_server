...
class AsyncSGDWorker : public ISGDCompNode {
 ...
 private:
  ...
  void ComputeGradient(int id) {
    mu_.lock();
    auto Y_raw = data_[id].first;
    auto Y = Y_raw->cols() > 1 ? Y_raw->safeColBlock(SizeR(0, 1)) : Y_raw;
    ...
    if (Y_raw->cols() > 1) {
      auto Y_w = Y_raw->safeColBlock(SizeR(1, 2));  
      loss_->compute({Y, Y_w, X, Xw.SMatrix()}, {grad.SMatrix()});
    } else {
