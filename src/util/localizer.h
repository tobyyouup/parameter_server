...
class Localizer {
 public:
  ...
  template<typename C>
   void CountUniqIndex_optimized_by_tanxu(
      const std::unordered_map<I, I>& idx, SArray<I>* uniq_idx, SArray<C>* idx_frq);
...
template<typename I, typename V>
template<typename C>
void Localizer<I,V>::CountUniqIndex_optimized_by_tanxu(
   const std::unordered_map<I, I>& idx, SArray<I>* uniq_idx, SArray<C>* idx_frq) {
  if (idx.empty()) return;
  CHECK(uniq_idx);

  std::unordered_map<I, I>& ugly_cast = const_cast<std::unordered_map<I, I>& >(idx);

  uniq_idx->clear();
  if (idx_frq) idx_frq->clear();

  std::vector<I> key_vector;
  for (auto it=idx.begin(); it != idx.end(); ++it) {
	  key_vector.push_back(it->first);
  }
  std::sort(key_vector.begin(), key_vector.end());

  I cnt_max = static_cast<I>(std::numeric_limits<C>::max());
  for(auto it = key_vector.begin(); it != key_vector.end(); ++it){
	  uniq_idx->push_back(*it);
	  C c_cnt = static_cast<C>(std::min(ugly_cast[*it], cnt_max));
	  idx_frq->push_back(c_cnt);
  }
}
