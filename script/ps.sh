...
start [options] bin args 	start an application
...
evenly assigned to these machines. #nodes could > #machines. if it is empty, generate a file with localhost ip
    -nw     number of worker nodes
    -ns     number of server nodes
    bin     	binary to launch, eg:../build/linear
    args 	other arguments, eg: -app_file l1lr.conf
...
check_start() {
    echo "use binary [$bin] and arg [$arg]"
    echo "starting $server servers and $worker workers"
	# if [[ ! $arg ]]; then
	#     show_empty '-arg'
	# fi
    local=0
...
get_port() {
	...
	ssh $1 "/usr/bin/env /usr/sbin/lsof -i tcp:$port" >> /dev/null
	while [[ $? -lt 1 ]]; do
		port=$(($RANDOM%(32767-1023)+1024))
		#ssh $1 "/usr/bin/env lsof -i tcp:$port" >> /dev/null
		ssh $1 "/usr/bin/env /usr/sbin/lsof -i tcp:$port"  >> /dev/null
